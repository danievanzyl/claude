...
1. **Local** — files exist in the current working directory → use Grep, Glob, LS
...
**NEVER traverse the local filesystem to find files from a remote repo.** If the target is remote, use `gh` commands — do NOT fall back to Grep/Glob on local paths.

### Key `gh` Commands for Remote Access
- `gh search code "query" --repo OWNER/REPO` — search code in a repo
- `gh api repos/OWNER/REPO/contents/PATH` — list directory or fetch file
